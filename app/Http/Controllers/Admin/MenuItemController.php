...
class MenuItemController extends Controller
{
    ...
    {
        $menu = $menuItem->menu;
        $allItems = $menu->items()->where('id','!=',$menuItem->id)->orderBy('parent_id')->orderBy('position')->get();
        return view('admin.menu_items.edit', compact('menuItem','menu','allItems'));
    }
    ...
    private function normalizeByType(array $data): array
    {
        ...
        if ($type !== 'url') $data['url'] = null;
        if ($type !== 'page') $data['page_id'] = null;
        if ($type !== 'post_category') $data['post_category_id'] = null;
        if ($type !== 'route') $data['route_name'] = null;
